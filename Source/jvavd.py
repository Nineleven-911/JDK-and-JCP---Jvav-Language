...
print("Jvav Decompiler is Running.")
...
# 打开指定的.jvav文件并读取内容
with open(jvav_file, "r") as f:
    jvav_content = f.read()
    # 拆分文本为行列表
    lines = jvav_content.split('\n')
    # 初始化一个空列表来存储替换后的每一行
    decompiled_lines = []
    # 遍历每一行进行替换，并打印替换信息
    for line in lines:
